...
def _copy_if_missing_or_newer(src: Path, dst: Path) -> bool:
    """Copy src -> dst if missing or src newer. Returns True if copied."""
    if not src.exists():
        return False
    if not dst.exists():
...
async def _async_init_once(hass: HomeAssistant) -> None:
    """Initialize storage, websocket API, panel and event listeners exactly once."""
    data = hass.data.setdefault(DOMAIN, {})
    if data.get("_initialized"):
        return
    ...
    # -----------------------------
    # Access processing (Events -> prüfen -> loggen -> pulse)
    # -----------------------------
    ...
    async def _on_esphome_input(ev: Event) -> None:
        ...
        # Feedback Helper (wenn vorhanden) - entity IDs are optional
        if granted:
...
async def async_setup(hass: HomeAssistant, config: dict) -> bool:
    # Allow YAML setup; initialize once.
    await _async_init_once(hass)
    return True


async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    # Config Flow setup; initialize once and forward platforms.
    await _async_init_once(hass)
